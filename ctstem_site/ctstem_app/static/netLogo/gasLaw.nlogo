...
to setup
  ...
  set piston-position wall-position - box-edge
  ...
  set total-particle-number num-particles
  reset-ticks
  recolor
end
...
to make-particles [number]
  ...
  ask particles with [who < num-particles]
    [
...
to set-num-particles
  ifelse num-particles > total-particle-number 
  [
    ask n-of (num-particles - total-particle-number) particles with [dark-particle? = true] [
      set dark-particle? false
      set shape "circle"
      recolor
    ]
  ...
  [
    ask n-of (total-particle-number - num-particles) particles with [dark-particle? = false] [
      set shape "nothing" 
      set dark-particle? true      
    ]
  ]
...
;   ask piston-wall  [ set pcolor piston-color ]
  ...
  ;; move particles that are outside the box back inside the box  
  ask particles with [pxcor >= (round piston-position - 1) ] [
    
;    set xcor (random-float (abs(((-1 * box-edge) + 1) - (piston-position - 1))) - (box-edge - 1))
    
    set xcor ((1 - box-edge)  + random-float (box-edge + piston-position - 3))
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if pressure-history != 0\n  [ \n  plotxy ticks (mean pressure-history) \n  ]"
